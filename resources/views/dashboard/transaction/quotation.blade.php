                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-4">
                                    ...
                                    <select class="form-control selects" id="status">
                                        <option></option>
                                        @foreach(Helpers::transactionStatus('quotation') as $s)
                                            <option>{{ $s }}</option>
                                        @endforeach
                                    </select>
                                </div>
    ...
    <script>
        ...
        var table = $("#dataTable").DataTable({
            ...
            columns: [
                ...
                {  
                    ...
                    mRender : function(data) {
                        var deadlineDate = "";
                        if(data.quote_status.name == "Menunggu Konfirmasi Pembeli"){
                            deadlineDate = "<br><small class='text-muted'>Batas Waktu Konfirmasi Pembeli "+moment(data.deadline_date).format('D MMMM Y LT')+"</small>"
                        }
                ...
                },
                {
                    data: null,
                    mRender: function(data) {
                        var btnUpdate = "Konfirmasi";
                        if(data.quote_status.name == "Negosiasi") {
                            btnUpdate == "Negosiasi";
                        }
                        ...
                        $opsiHtml += '@can("transaction.quotation_view")<li><a class="dropdown-item" href="{{ url("dashboard/quotation") }}/'+data.number+'">Lihat</a></li>@endcan';
                        $opsiHtml += '@can("transaction.quotation_update")<li><a class="dropdown-item" href="{{ url("dashboard/quotation") }}/'+data.number+'/update">'+btnUpdate+'</a></li>@endcan';
                        $opsiHtml += '@can("transaction.quotation_delete")<li><a class="dropdown-item" onclick="deleted('+data.id+')" href="#">Delete</a></li>@endcan';
                        $opsiHtml += '</ul>';
                        return $opsiHtml;
                    }
                },
        ...
        deleted = (id) => {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url : `{{ url('dashboard/quotation/${id}') }}`,
                        type : 'DELETE',
                        dataType: 'JSON',
                        data : { 
