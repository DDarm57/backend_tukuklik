@extends('layouts.dashboard.app', ['title' => 'Varian'])
    ...
    <section class="section">
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">List Varian</h5>
                        @can('product.attribute_create')
                            <a href="{{ url('dashboard/attribute/create') }}" class="btn btn-outline-danger mt-2 mb-4"><i class="bi bi-plus-circle"></i> Create Varian Attribute</a>
                        @endcan
    ...
    <script>
            ...
            columns: [
                ...
                {  
                    ...
                    mRender : function(data) {
                        ...
                        ? 
                        `<div style="border-radius:15px;" class="bg-success p-1 text-white text-center">${data.status}</div>` 
                        : 
                        `<div style="border-radius:15px;" class="bg-danger p-1 text-white text-center">${data.status}</div>`
                    }
                },
                { 
                    data : null,
                ...
                },
                {
                    ...
                    mRender: function(data) {
                        ...
                        $opsiHtml += '@can("product.attribute_update")<li><a class="dropdown-item" href="{{ url("dashboard/attribute") }}/'+data.id+'/edit">Update</a></li>@endcan';
                        $opsiHtml += '@can("product.attribute_delete")<li><a class="dropdown-item" onclick="deleted('+data.id+')" href="#">Delete</a></li>@endcan';
                        $opsiHtml += '</ul>';
                        return $opsiHtml;
                    }
                },
        ...
        deleted = (id) => {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url : `{{ url('dashboard/attribute/${id}') }}`,
                        type : 'DELETE',
                        dataType: 'JSON',
                        data : { 
