<html>
    ...
    <head>
        <title>Pesanan {{ Helpers::generalSetting()->system_name. " - ". $order->order_number }}</title>
    ...
    <body>
        <div class="container clearfix header">
            <table width="100%">
                <tr>
                    ...
                    <td class="valign-top">
                        <div>
                            <h2 style="font-size:20px;text-align:right">Purchase Order</h2>
                        </div>
                    </td>
                </tr>
            </table>
                ...
                <tr>
                    <td class="valign-top" width="30%">
                        ...
                        <table style="margin-left: -3px">
                            <tr>
                                <td><b>{{ $order->quotation->user_pic }}</b></td>
                                <td>
                                </td>
                            </tr>
                        </table>
                        <p>{{ strtoupper($shippingAddress) }}</p>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        ...
                        <p>{{ strtoupper($merchantAddress) }}</p>
                    </td>
                </tr>
            </table>
        ...
        <div class="container">
                            ...
                            <table width="100%" cellpadding="5">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{{ $order->quotation->purpose_of }}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @php 
                                                $notes = wordwrap(strip_tags($order->quotation->notes_for_merchant), 50, "\n");
                                            @endphp
                                            {!! $notes !!}
                                        </td>
                                    </tr>
            ...
            @if($order->status->name == "Kadaluwarsa")
                <div class="bordered" style="padding: 5px;">
                    <strong> Alasan Kedaluwarsa :</strong> {{ $order->quotation->optional_status }}
                </div>
            @endif

            @if($order->status->name == "Ditolak Penjual")
                <div class="bordered" style="padding: 5px;">
                    <strong> Alasan Ditolak Penjual :</strong> {{ $order->quotation->optional_status }}
                </div>
            @endif
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td colspan="4" style="text-align: left;">
                                        Jasa Pengiriman : 
                                        {{ $order->quotation->shippingRequest->shippingMethod->method_name ?? 'Menunggu Konfirmasi Penjual' }}
                                    </td>
                                    <td colspan="3">
                                ...
                                <table class="page-break" width="30%" cellpadding="5" cellspacing="0" align="right"
                                    ...
                                    <tbody>
                                        <tr>
                                            <th>Sub Total</th>
                                            <td>Rp.</td>
                                            <td class="text-right">{{ number_format($order->quotation->subtotal,2,'.',',') }}</td>
                                        </tr>
                                        <tr class="tax-total">
                                            <th>
                                                PPN - {{ ($order->quotation->tax_amount / $order->quotation->subtotal * 100) ?? 0 }}%
                                            </th>
                                            <td>Rp.</td>
                                            <td class="text-right">
                                                {{ number_format($order->quotation->tax_amount,2,'.',',') }}
                                            </td>
                                        </tr>
                                        <tr class="tax-total">
                                            <th>
                                                PPH - {{ ($order->quotation->income_tax / $order->quotation->subtotal * 100) ?? 0 }}%
                                            </th>
                                            <td>Rp.</td>
                                            <td class="text-right">
                                                ({{ number_format($order->quotation->income_tax,2,'.',',') }})
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>Ongkos Kirim</th>
                                            <td>Rp.</td>
                                            <td class="text-right">{{ number_format($order->quotation->shipping_amount,2,'.',',') }}</td>
                                        </tr>
        ...
        margin: 2px; font-style: italic;">
            ...
            <br/><br/>
            <b style="margin-top:50px;">Menyetujui</b>
            <br/><br/>
            <br/><br/>
            <br/>
            {{ $order->quotation->user->name }}
            @if(isset($order->quotation->user->lpse))
                <br/>
                <p>Pejabat Pengadaan {{ ucwords($order->quotation->user->lpse->nama_instansi) }}</p>
                <p>({{ ucwords(strtolower($order->quotation->user->lpse->nama_satker)) }})</p>
            @endif
        </div>
