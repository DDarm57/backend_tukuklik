<html>
    ...
    <head>
        <title>Tagihan {{ Helpers::generalSetting()->system_name. " - ". $invoice->invoice_number }}</title>
    ...
    <body>
        ...
        <div class="container clearfix header">
            ...
            <table width="100%">
                <tr>
                    ...
                    <td class="valign-top">
                        <div>
                            <h2 style="font-size:20px;text-align:right">Invoice</h2>
                        </div>
                    </td>
                </tr>
            </table>
                        ...
                        <td>
                            <table width="100%" cellpadding="3">
                                <tbody>
                                    <tr>
                                        <th>Penjual</th>
                                        <td>{{ $invoice->order->quotation->merchant->name }}</td>
                                    </tr>
                                    <tr>
                                        <th>Pembeli</th>
                                        <td>{{ $invoice->order->quotation->user->name }}</td>
                                    </tr>
                                    <tr>
                                        <th>No. PO</th>
                                    ...
                                    <tr>
                                        <th>Tipe Tagihan</th>
                                        <td>{{ $invoice->order->quotation->payment_type }} {{ $invoice->order->quotation->payment_type == "Term Of Payment" ? " ".$invoice->order->quotation->termin." Hari" : null }}</td>
                                    </tr>
                                    <tr>
                        ...
                        <td>
                            <table width="100%" cellpadding="5">
                                <tbody>
                                    ...
                                    <tr>
                                        <th>Metode Bayar</th>
                                        <td>{{ $invoice->paymentMethod->payment_name ?? '-' }}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <th>{{ $invoice->paymentMethod->payment_service == "Manual" ? "Tujuan Transfer" : "No. Virtual Account" }}</th>
                                        <td>{{ $invoice->billing_number }} {{ $invoice->paymentMethod->payment_service == "Manual" ? " Atas Nama " : "" }} {{ $invoice->paymentMethod->account_holder ?? ''  }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td colspan="4" style="text-align: left;">
                                        Jasa Pengiriman : 
                                        {{ $invoice->order->quotation->shippingRequest->shippingMethod->method_name ?? 'Menunggu Konfirmasi Penjual' }}
                                    </td>
                                    <td colspan="3">
                                        Pengiriman dari lokasi penjual : {{ $invoice->order->quotation->merchant->address->city->city_name }} </td>
                                </tr>
                            </tbody>
                        </table>
                                ...
                                <table class="page-break" width="30%" cellpadding="5" cellspacing="0" align="right"
                                    ...
                                    <tbody>
                                        <tr>
                                            <th>Sub Total</th>
                                            <td>Rp.</td>
                                            <td class="text-right">{{ number_format($invoice->order->quotation->subtotal,2,'.',',') }}</td>
                                        </tr>
                                        <tr class="tax-total">
                                            <th>
                                                PPN - {{ ($invoice->order->quotation->tax_amount / $invoice->order->quotation->subtotal * 100) ?? 0 }}%
                                            </th>
                                            <td>Rp.</td>
                                            <td class="text-right">
                                                {{ number_format($invoice->order->quotation->tax_amount,2,'.',',') }}
                                            </td>
                                        </tr>
                                        <tr class="tax-total">
                                            <th>
                                                PPH - {{ ($invoice->order->quotation->income_tax / $invoice->order->quotation->subtotal * 100) ?? 0 }}%
                                            </th>
                                            <td>Rp.</td>
                                            <td class="text-right">
                                                ({{ number_format($invoice->order->quotation->income_tax,2,'.',',') }})
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>Ongkos Kirim</th>
                                            <td>Rp.</td>
                                            <td class="text-right">{{ number_format($invoice->order->quotation->shipping_amount,2,'.',',') }}</td>
                                        </tr>
                                        <tr class="transaction_total bordered">
                                            <th>Total Tagihan</th>
                                            <th>Rp.</th>
                                            <th class="text-right"> {{ number_format($invoice->order->grand_total,2,'.',',') }}</th>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
        ...
        margin: 2px; font-style: italic;">
                        Terbilang : {{ Helpers::countedAmount($invoice->order->grand_total) }} </div>
