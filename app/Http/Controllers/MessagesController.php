...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        $fetch->name = $fetch->merchant->name ?? $fetch->name;
        if($fetch){
    ...
    public function send(Request $request)
    {
        ...
        $userTo = User::find($request['id']);

        $fromName = Auth::user()->merchant->name ?? Auth::user()->name;
        ...
        $notifData = [
            ...
            'subject'   => 'Pesan Baru Dari '. $fromName,
            'url'       => url('chat/'.Auth::user()->id.'')
        ];

        Notification::send(User::find($request['id']), new MessageSent($notifData));
    ...
    public function getContacts(Request $request)
    {
        ...
        ->selectRaw('
            ...
            MAX(ch_messages.created_at) max_created_at
        ')
        ...
        $users->map(function($n) {
            ...
            if($n->merchant_name != null) {
                $n->name = $n->merchant_name;
            }
        });
        ...
        if (count($usersList) > 0) {
            ...
            $contacts = '<p class="message-hint center-el"><span>No messages found</span></p>';
        }
    ...
    public function countUnseenMessage(Request $request)
    {
        return Response::json([
            'data' => auth()->user()->un_seen_messages
        ], 200);
    }
}
